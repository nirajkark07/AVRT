...
class YcbineoatReader:
  ...
  # # Modified mask function to take custom mask as input
  # def get_mask(self,i):
  #   mask = cv2.imread(f'{self.video_dir}/masks/mask.png', -1)
  #   if len(mask.shape)==3:
  #     for c in range(3):
  #       if mask[...,c].sum()>0:
  #         mask = mask[...,c]
  ...
  def get_depth(self,i):
    depth = cv2.imread(self.color_files[i].replace('rgb','depth'),-1)*0.0002500000118743628 # Multiply by the depth scale
